window.onload = () => {
	if (!window.jQuery || typeof window.jQuery == 'undefined')
		((_e, _url) => {
			let s = document.createElement(_e)
			s.type = 'text/javascript'
			s.async = true
			s.src =
				(document.location.protocol == 'https' ? 'https://' : 'http://') + _url
			let t = document.getElementsByTagName(_e)[0]
			t.parentNode.insertBefore(s, t)
		})('script', 'ajax.googleapis.com/ajax/libs/jquery/3.1.1/jquery.min.js')
	setTimeout(() => {
		window.jQuery && typeof window.jQuery != 'undefined'
			? ((jQuery) => {
					...
					jQuery(document).ready(($) => {
						...
						// Login modal animations and logic.
						{
							...
							$('.animation-wrapper > .content').each((_index, _el) => {
								...
								_el
									.find('.animation-control.backward')
									.each((_index, _ctrl) => {
										$(_ctrl).on('click', () => {
											if (!_el.is(':first-of-type')) {
						...
						$('.has-selection-box').each((_index, _el) => {
							...
							_el
								.find('.selection-box .dismiss-selection')
								.on('click', (_e) => {
									_e.preventDefault()
									setTimeout(() => {
			...
			: console.error(
					'Could not load jQuery!\nMake sure jQuery is loaded before importing this script.'
			  )
	}, 200) // Timout value depends upon the network speed
}

const _openModal = (_s) => {
	;(($) => {
		let _modal = $(_s)
		let _overlay = _modal.parent()
